...
This is 'kumzzy' practice work from the Cyfrin Foundry Solidity Course.
...
```
git clone https://github.com/kumzzy/foundry-smart-contract-lottery-kumzzyf25
cd foundry-smart-contract-lottery-kumzzyf25
forge build
```
...
If you're having a hard time installing the foundry devops , you can optionally run this command.
```
...
You'll want to set your `SEPOLIA_RPC_URL`, `SENDER_ADDRESS` and `WALLET_NAME` as environment variables. You can add them to a `.env` file, and use the keystore function for your private key
```
cast wallet import <wallet Name> --interactive
```

- `SENDER_ADDRESS `: The address of your account (like from [metamask](https://metamask.io/)). **NOTE:** FOR DEVELOPMENT, PLEASE USE A KEY THAT DOESN'T HAVE ANY REAL FUNDS ASSOCIATED WITH IT.
...
```
cast send <RAFFLE_CONTRACT_ADDRESS> "enterRaffle()" --value 0.1ether --account <$ACCOUNT_NAME> --rpc-url $SEPOLIA_RPC_URL
```

or, to create a ChainlinkVRF Subscription:

```
make deploy-createSubscription
```

or, to fund a ChainlinkVRF Subscription:

```
make deploy-fundSubscription
```
or, to addConsumer:

```
make deploy-addConsumer
```
...
let's connect on twitter [https://x.com/kumzzyy]